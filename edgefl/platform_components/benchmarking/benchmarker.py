...
class Benchmarker:
    def __init__(self, endpoint:str, db_name:str = "benchmarkfl", table_name: str = "fl_benchmarks", enabled: bool = True):
        ...
        if not self._verify_target_availability():
            self.enabled = False
            logger.warning(
                f"Benchmark target {self.endpoint} has no Operator process running and "
                "can't ingest data. DISABLING BENCHMARKING, point BENCHMARK_REST_CONN "
                "at an operator node."
            )
            return
    ...
    def _ensure_dbms_connected(self):
        try:
            ...
            if self.header["dbms"] not in connected:
                logger.info(f"Benchmark dbms missing; connecting {self.header['dbms']}")
                self._connect_sqlite_dbms(self.header["dbms"])
        except Exception as e:
            logger.warning(f"Benchmarker dbms connection failed: {e}")
    ...
    def _verify_target_availability(self) -> bool:
        ...
        try: 
            ...
            logger.warning(f"Benchmarker could not probe target {self.endpoint}: {e}")
            return False
        ...
        while True:
            ...
            try:
                ...
                if response.status_code >= 400:
                    logger.warning(
                        f"Benchmarker PUT failed: {response.status_code} {response.text}"
                    )

            except requests.exceptions.RequestException as e:
                logger.error(f"Benchmarker PUT error: {e}")
            except Exception as e:
                logger.error(f"Benchmarker unexpected error: {e}", exc_info=True)
            finally:
                self.q.task_done()
